...
function extractPropertyData() {
  ...
  if (url.includes('591.com.tw')) {
    return extract591Data();
  }
  
  // 信義房屋
  if (url.includes('sinyi.com.tw')) {
    return extractSinyiData();
  }
  
  // 永慶房仲
  if (url.includes('yungching.com.tw')) {
    return extractYungchingData();
  }
  ...
  try {
    ...
    if (addressEl) {
      ...
      if (cityMatch) {
        ...
        const districtMatch = data.address.match(/市|縣)(.+?區|鎮|市|鄉)/);
        if (districtMatch) {
